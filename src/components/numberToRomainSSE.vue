<template>
  <div class="container-lg">
    <div class="row justify-content-center">
      <form class="col-md-4 bg-light shadow p-4 mt-4" id="app" v-on:submit="convertNumber" ref="form">
        ...
        <p class="alert alert-success pb-3 w-100" v-if="Object.keys(response).length>0 && response.error === false" v-text="'Roamin of '+form.number+' is ' +response.result"></p>
        ...
        <div class="w-100">
          ...
          <input class="w-25 m-2 btn btn-sm btn-success float-lg-end" type="submit" value="submit">
        </div>
      </form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    convertNumber: async function (e) {
      e.preventDefault();
      this.response = {};
      this.response = await axios.post(process.env.VUE_APP_API_URL + "/sse/numberToRomain", this.form)
    },
    cancel: function(){
      this.response = {};
